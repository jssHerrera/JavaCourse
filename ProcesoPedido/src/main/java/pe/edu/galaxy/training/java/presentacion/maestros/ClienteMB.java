...
import pe.edu.galaxy.training.java.service.impl.PaisServiceImpl;
import pe.edu.galaxy.training.java.service.inf.ClienteService;
import pe.edu.galaxy.training.java.service.inf.PaisService;
...
public class ClienteMB extends GenericoMB {
	...
	private boolean validar() {
		...
		if (!(razonSocial != null && razonSocial.trim().length() > 5)) {
			super.msgAlert("La razon social es requerida y debe tener minimo 5 caracteres");
			return false;
		}

		Long idPais = this.getCliente().getPais().getId();
		if (!(idPais != null && idPais > 0)) {
			super.msgAlert("El pais es requerido");
			return false;
		}
		return true;
	}
