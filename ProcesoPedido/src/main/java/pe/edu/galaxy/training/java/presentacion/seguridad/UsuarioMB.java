...
import pe.edu.galaxy.training.java.service.impl.UsuarioServiceImpl;
import pe.edu.galaxy.training.java.service.inf.UsuarioService;
...
public class UsuarioMB extends GenericoMB{
	...
	private boolean validar() {
		...
		if (!(nombre!=null && nombre.trim().length()>5)) {
			super.msgAlert("El nombre es requerido y debe tener m�nimo 5 caracteres");
			return false;
		}
		
		String usuario=this.getUsuario().getUsuario();
		if (!(usuario!=null && nombre.trim().length()>3)) {
			super.msgAlert("El usuario es requerido y debe tener m�nimo 3 caracteres");
			return false;
		}
		return true;
	}
